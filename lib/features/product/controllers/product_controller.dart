...
class ProductController extends ChangeNotifier {
  ...
  Future<void> getSelectedProductModel(int offset, {bool isUpdate = true, bool reload = false}) async {
    if (offset == 1 && !reload && _selectedProductModel != null && (_selectedProductModel!.products?.isNotEmpty ?? false)) {
      return;
    }
    ...
    if(offset == 1) {
      DataSyncHelper.fetchAndSyncData(
        ...
        skipRemoteWhenLocalExists: !reload,
  ...
  Future<void> getLatestProductList(int offset, {bool isUpdate = false, bool reload = false}) async {
    if (offset == 1 && !reload && _latestProductModel != null && (_latestProductModel!.products?.isNotEmpty ?? false)) {
      return;
    }
  ...
  Future<void> getFeaturedProductModel(int offset, {bool isUpdate = false, bool reload = false}) async {
    if (offset == 1 && !reload && _featuredProductModel != null && (_featuredProductModel!.products?.isNotEmpty ?? false)) {
      return;
    }
  ...
  Future<void> getRecommendedProduct({bool reload = false}) async {
    if (!reload && _recommendedProduct != null) {
      return;
    }
  ...
  Future<void> getClearanceAllProductList(int offset, {bool isUpdate = true, bool reload = false}) async {
    if (offset == 1 && !reload && _clearanceProductModel != null && (_clearanceProductModel!.products?.isNotEmpty ?? false)) {
      return;
    }
